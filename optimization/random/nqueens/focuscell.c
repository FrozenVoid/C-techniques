...
u64 limlq=128/log2index(N);
u64 fq=firstq(q,N),best=fq,cur,lastq=0, A,B;
while(fq<(N-2) && lastq<limlq){
B=fq;//force focus cell to swap queens
do{A=randuint64()%N;}while(!(B^A));
swapq(q[A],q[B]);
cur=firstq(q,N);//count diagonal intersects
if(cur<B){lastq++; //revert if worse
swapq(q[A],q[B]);continue;  }
fq=firstq(q,N);lastq=0;
//new record
#if QDEBUG
clock_t curt=clock();
  if(curt-startt2> INTERSECT_DISP*CLOCKS_PER_SEC){
  print("Time(s):",(curt-startt)/CLOCKS_PER_SEC,"Complete:",B,"/",N,"\n");startt2=curt;}
#else
